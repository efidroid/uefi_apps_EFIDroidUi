...
#define die( str ) {fprintf(stderr, str "\n" ); return  -1;}
#define CHECK(exp) {int rc = (exp);if(rc)return rc;}
...
static uint8_t* volume_buffer = NULL;
static uint32_t volume_buffer_pos = 0;
...
static int setup_tables(void)
{
    ...
    CHECK(mark_FAT_cluster(0, 0xffffffff));	/* Initial fat entries */
    CHECK(mark_FAT_cluster(1, 0xffffffff));
    fat[0] = (unsigned char)bs.media;	/* Put media type in first byte! */
    if (size_fat == 32) {
	/* Mark cluster 2 as EOF (used for root dir) */
	CHECK(mark_FAT_cluster(2, FAT_EOF));
    }
...
#define seekto(pos,errstr)						\
  do {									\
    volume_buffer_pos = pos; \
  } while(0)

#define writebuf(buf,size,errstr)			\
  do {							\
    int __size = (size);				\
    memcpy(volume_buffer+volume_buffer_pos, buf, __size); \
    volume_buffer_pos += __size; \
  } while(0)
...
)
{
    ...
    volume_buffer = buffer;

    devinfo.type = TYPE_FILE;
	devinfo.partition = 0;
	devinfo.size = buffersize;
    ...
    if(setup_tables())		/* Establish the filesystem tables */
        return EFI_DEVICE_ERROR;
    if(write_tables())		/* Write the filesystem tables away! */
        return EFI_DEVICE_ERROR;
